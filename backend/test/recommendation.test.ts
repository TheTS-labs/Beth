...
import { seed as seedUsers } from "../db/seeds/01_user_permission";
import { seed as seedPosts } from "../db/seeds/02_post";
...
describe("POST /recommendation/recommend", () => {
  it("should return any recommendations", async () => {
    // Preparing
    await seedUsers(server.db);
    await seedPosts(server.db);
